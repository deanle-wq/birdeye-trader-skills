...
Answer this daily trader job: **What unusually large trades hit this token recently?**
...
Learn the Large Token Trades skill.

From this repository, install the public package:

```bash
npx skills add https://github.com/deanle-wq/birdeye-trader-skills --skill birdeye-large-trade-scanner
```
...
- Fetch recent indexed token transactions and order the observable trades by USD size.
- Preserve direction, wallet, token amount, USD value and timestamp when returned.
- Keep the scan window, page limit and any ambiguous transaction direction visible.

## Just say to your agent

- `What were the biggest trades in <token address> during the last hour?`
- `Show the 20 largest recent indexed trades for this token.`
...
1. `birdeye-cli token large-trades --token <token_address>`
...
| Field | Type | Meaning |
|---|---|---|
| `large-token-trades.trades` | `array[market-trade]` | Trades. |
| `large-token-trades.returned_count` | `integer` | Returned count. |
| `large-token-trades.min_volume_usd` | `decimal-string|null` | Min volume usd. |
| `large-token-trades.has_next` | `boolean|null` | Has next. |
| `large-token-trades.ordering` | `string` | Ordering. |
...
- Internal dependencies: `large-token-trades`.
