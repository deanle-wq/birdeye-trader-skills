...
Answer this daily trader job: **What tokens were listed most recently?**
...
Learn the Newly Listed Tokens skill.

From this repository, install the public package:

```bash
npx skills add https://github.com/deanle-wq/birdeye-trader-skills --skill birdeye-newly-listed-tokens
```
...
- Return the latest Solana tokens indexed by Birdeye.
- Show listing time and the available token identity and market fields.
- Keep result count, pagination and the provider's listing coverage visible.

## Just say to your agent

- `What tokens were listed most recently on Solana?`
- `Show me the latest 10 Birdeye token listings.`
...
1. `birdeye-cli market new-listings`
...
| Field | Type | Meaning |
|---|---|---|
| `newly-listed-tokens.tokens` | `array[new-token]` | Tokens. |
| `newly-listed-tokens.returned_count` | `integer` | Returned count. |
| `newly-listed-tokens.limit` | `integer` | Limit. |
...
- Internal dependencies: `newly-listed-tokens`.
