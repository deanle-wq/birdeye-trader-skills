...
Answer this daily trader job: **What did this wallet trade in the last 24 hours?**
...
Learn the Wallet Activity Feed skill.

From this repository, install the public package:

```bash
npx skills add https://github.com/deanle-wq/birdeye-trader-skills --skill birdeye-wallet-activity-feed
```
...
- Fetch one wallet's latest indexed activity for the last 24 hours by default.
- Show token, direction, amount, USD value and timestamp when available.
- Allow a different bounded time window and transaction filters while preserving pagination coverage.

## Just say to your agent

- `What did <wallet address> trade in the last 24 hours?`
- `Show this wallet's latest buys today.`
...
1. `birdeye-cli track wallet --wallet <wallet_address>`
...
- Category: `Monitor`; trader stage: `MONITOR`; difficulty: `Basic`.
